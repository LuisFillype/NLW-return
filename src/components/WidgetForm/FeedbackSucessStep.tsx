...
interface FeedbackContentSucessProps {
  onFeedbackRestartResquested: () => void;

}
export function FeedbackSucessStep({ onFeedbackRestartResquested }: FeedbackContentSucessProps) {
  return (
    <>
      <header>
        <CloseButton />
      ...
      <div className="flex flex-col items-center py-10 w-[304px]">
        ...
        <button
          ...
          className="py-2 px-6 mt-6 bg-zinc-800 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-700 focus:outline-300 focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-brand-500 transition-colors focus:outline-none disabled:opacity-50 disabled:hover:bg-brand-500">Quero enviar outro</button>
      </div>
    </>);
}